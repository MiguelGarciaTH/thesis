...
These strategies are build from the data that is available on vulnerability database developed in a previous work.
The promising results confirm the intuition that applying diversity implies vulnerability independence to some extent.
In this contribution, it should be clear what are the security gains from using different \glspl{os} on a replicated intrusion-tolerant system.
...
Contrary to automatic diversity mechanisms that needs testing to assess the vulnerability correlation, \gls{ots} diversity can be supported by data evidence to understand the correlation between vulnerabilities and products. 
