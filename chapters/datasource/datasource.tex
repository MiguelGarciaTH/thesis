\chapter{A Study on Common OS Vulnerabilities}
\label{chap:datasource}
This chapter presents a data analysis that gives evidence to support our thesis.
In particular, we describe three strategies to select different \glspl{os} to be part of a \gls{bft} replicated system.
These strategies leverage on the vulnerability database developed in a previous work.
Contrary to automatic diversity, \gls{ots} diversity can be supported by data evidence to understand the correlation between vulnerabilities and products. 
Therefore, we focus on the diversity of \glspl{os} (not only the kernel but the whole product) for three fundamental reasons: (1) by far, most of the replica’s code is the \gls{os}; (2) such size and importance, make \glspl{os} a valuable target, with new vulnerabilities and exploits being discovered every day; and (3) there are many options of \glspl{os} that can be used.
The promising results confirm the intuition that applying \gls{os} diversity implies vulnerability independence to some extent.
...
\begin{defn}
 \emph{``[...] A vulnerability is a weakness in an information system, system security procedures, internal controls, or implementation that could be exploited by a threat source.''}~\cite{Nist:2012}
\end{defn}
...
Second, it lets us modify the \gls{cve} fields to correct problems.
...
The following steps were done to keep the database updated.
We selected 2563 vulnerabilities from a total of more than 44,000 vulnerabilities published by \gls{nvd} in the period of 1994 to 2011.
...
In the context of this thesis, we are focused on the dependability of replicated systems.
The starting point of this analysis are vulnerabilities that were shared in \gls{os} pairs.
In this study, we consider two possible server machine setups offering an increasingly more secure platform. 
...
In practice, when considering the second configuration, at least one application (e.g., name service or a distributed file service) would be deployed in an intrusion-tolerant system, and the vulnerabilities of this application would add up to the flaws that we will report. 
...
The largest group that was affected by the same vulnerability had six \glspl{os}, and this occurred only for a single flaw.
There were also two vulnerabilities in sets with five \glspl{os}. Vulnerabilities in two and three \glspl{os} usually occur in systems from the same family, whose common ancestry implies the reuse of more significant portions of the code base.
...
Table \ref{tab:spreaded_vulns} describes in more detail the vulnerabilities shared by larger groups (four to six) of \glspl{os}. 
The first three vulnerabilities have a considerable impact because they allow a remote adversary to run arbitrary commands on the local system using a high-privilege account. 
...
A closer look at the corresponding \gls{nvd} entry reveals external references to the sites of OpenBSD, NetBSD, and FreeBSD, indicating that they could be vulnerable in case this software is being used. 
...
The remaining three vulnerabilities are related to the protocol stack of TCP/IP. 
All of them affect system availability, allowing different forms of \gls{dos} attacks. 
CVE-2008-4609 is the vulnerability that affects more \glspl{os} according to the \gls{nvd} database, because TCP/IP stack code is often reused across \glspl{os}.%, we checked the websites of the other \glspl{os} for reports related to this vulnerability. 
...
A good portion of them are in the TCP/IP stack implementation, which is probably the most shared software component across \glspl{os}.
In addiction, they only impact on the availability of the system, leaving confidentiality and integrity of data unharmed.


\section{Strategies for OS Diversity Selection}\label{sec:datasource_evaluation}
The preliminary analysis presented in the previous section indicates that it is possible to find that some \glspl{os} share fewer vulnerabilities than others. 
...
This indicator is calculated for a given year $y$, based on the vulnerabilities that were shared by \glspl{os} $\mathit{A}$ and $\mathit{B}$ over a period of $\mathit{tspan}$ previous years. \gls{cvi} is built to ensure the following desirable properties:
\begin{enumerate}
\newcommand{\OLDtheenumi}{\theenumi}
\renewcommand{\theenumi}{\roman{enumi}}
\item $\mathit{CVI}_y(A,B) = 0$ if $\mathit{A}$ and $\mathit{B}$ have no common vulnerabilities in the $\mathit{tspan}$ interval before year $\mathit{y}$;
\item $\mathit{CVI}_y(A,B) < \mathit{CVI}_y(C,D)$ if $\mathit{A}$ and $\mathit{B}$ shared less vulnerabilities than $\mathit{C}$ and $\mathit{D}$ in each year of the considered period;
\item $\mathit{CVI}_y(A,B) < \mathit{CVI}_y(C,D)$ if $\mathit{A}$ and $\mathit{B}$ had $N$ common vulnerabilities in the distant past while $\mathit{C}$ and $\mathit{D}$ had $N$ shared vulnerabilities more recently;
\item $\mathit{CVI}_{y_2}(A,B) < \mathit{CVI}_{y_1}(A,B)$, with $y_1 < y_2$, if the number of common vulnerabilities for $\mathit{A}$ and $\mathit{B}$ has decreased over the years.
...
First, a weighting factor $\alpha_{i}$ is defined for each year   $i \in \{y-\mathit{tspan}+1, ..., y-2, y-1, y\}$ as defined in Equation~\ref{eq:tspan}:
...
Then, \gls{cvi} is obtained using the number of vulnerabilities $v_{i}(A,B)$ that appeared in both \glspl{os} $\mathit{A}$ and $\mathit{B}$ for every year $i$ from the start of the time span up to reference year $y$ (see Equation~\ref{eq:cvi}).
...
In the Isolated Thin Server case, there are three pairs with $\mathit{CVI}(A,B)=0$, which shows that they have no shared vulnerabilities during the past 12 years, and are thus particularly good candidates to include in intrusion-tolerant configurations. 
...
Indicators like \gls{cvi} can be useful to devise strategies to select diverse sets of \glspl{os}. 
...
It is indicated when one wants to give greater importance to more recent vulnerabilities. 
...
For every strategy, we present examples of \gls{os} sets for the Fat Server and Isolated Thin Server configurations. 
...
One should take into account that Ubuntu and Windows2008 were first released in 2004 and 2008 respectively, so the data for these two \glspl{os} was collected for a smaller number of years. 
...
OpenBSD & - & 33 & 43 & 9 & 2 & 3 & 10 & 3 & 2 & 1&   \multirow{13}{1mm}{\begin{sideways}\hspace{8mm}\parbox{19mm}{2000-2009}\end{sideways}} \\ \cline{1-11}
...
\begin{sideways}\vspace{2mm}\parbox{3mm}Win2008  \end{sideways}&
\multicolumn{2}{|c}{} \\ \cline{1-11}  \cline{1-12}
OpenBSD & - & 13 & 26 & 5 & 0 & 0 & 3 & 3 & 3 & 1&\multirow{13}{1mm}{\begin{sideways}\hspace{8mm}\parbox{19mm}{2000-2009}\end{sideways}} \\ \cline{1-11}
...
\subsection{Common Vulnerability Indicator Strategy (CVIst)} 
...
\begin{sideways}\vspace{2mm}\parbox{3mm}Win2008  \end{sideways}&
\multicolumn{2}{|c}{} \\ \cline{1-11}  \cline{1-12}
OpenBSD & - & 17.1 & 22.2 & 4.0 & 1.7 & 2.5 & 5.0 & 1.8 & 1.5 & 0.9 & \multirow{13}{1mm}{\begin{sideways}\hspace{8mm}\parbox{19mm}{$\mathit{CVI}_{2009}$(A,B)}\end{sideways}} \\ \cline{1-11}
...
The Venn diagrams for these two sets are shown in Figures~\ref{fig-venn_b}(a) and~\ref{fig-venn_b}(b).
...
By checking the data for the observed period, one can see that both sets do not share a single vulnerability, which indicates that the strategy would have made a good selection of \glspl{os} (see also the Venn diagrams in Figures~\ref{fig-venn_b}(c) and~\ref{fig-venn_b}(d)).


\begin{figure}[!t]
 \centering
 \includegraphics[scale=0.7]{images/images/grayscale_ven_dia_one_fig_update_b_2.pdf}
...
\subsection{Inter-Reporting Times Strategy (IRTst)} \label{IRT} 
...
\subsection{Comparing the three strategies}
...
As a result, if we consider (\gls{os}, release) pairs, one may augment the number of different systems that do not share vulnerabilities. 
...
The main goal of this thesis is to develop dependable \gls{bft} systems.
Therefore, one first step towards that goal is to explore the diversity of a particular \emph{4-diverse} set in the Isolated Thin Server configuration.
...
Despite the year of the release, since some vulnerabilities can be inherited from older versions of the code, we will include all vulnerabilities no matter the published date (i.e., even flaws discovered before 2000).
...
The first observation is that there are many pairs of releases within this set of 4 \glspl{os} that appear to be free of common flaws. 
...
In any case, it seems that automated diversity management, described in the next chapter, is needed. 
...
%It is worth to notice that the main manual work of analyzing the 2270 vulnerabilities of eleven \gls{os} distributions was done before this thesis~\cite{Garcia:2012}. 
We developed three strategies to deploy diverse sets, some of which will be used (in part) in Chapter~\ref{chap:lazarus_design}.
In particular, the \gls{cvis}, as it introduces the notion of time evolution, is fundamental for the following contributions.
This chapter contributes to answer one of the main questions that drove our research: 
