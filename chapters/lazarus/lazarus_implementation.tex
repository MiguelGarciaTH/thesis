\chapter{\system}
\textbf{*************TO IGNORE*********** jump to Descentralized Lazarus Version}
...
We have designed a new architecture for replicated \system, an overview of the architecture is shown in the Figure~\ref{fig:lazarus_distributed}. 
In this new architecture, we have two instances of \gls{smr}.
The first one runs a specific service in the \emph{Execution Plane}, and it was already presented in Chapter~\ref{chap:lazarus_design}.
...
Each \gls{ltu} run a \gls{bft} library client (i.e., \textsc{BFT-SMaRt} client). 
...
We adopted the Syta~\etal{}~\cite{Syta:2017} soltuion named \textsc{RandShare}, as it guarantees unbiasability, unpredictability, and availability on random number generation.
...
This approach would minimize the use of resources, as each \emph{Controller Replica} would store only parts of the complete.
...
That said, a problem is raised as it is a challenge to create \gls{vm} images that are equal.
