...
This modification introduces some significant modifications to the system model and architecture which are explained below.

\paragraph{System Model}
This version \system shares some similarities with the centralized version.
For instance, the \emph{Execution Plane} remains practically equal.
...
Contrary to the usual \gls{smr}, that works as a request-response, in this case, \emph{Controller Proxy} receives messages without a prior request from them. 


\paragraph{Replica Synchronization Protocol}
In the centralized version, \system node needs to make updates to the database. 
Thus it decides when it is time to crawl \gls{osint} sources to look for new data.
Moreover, as the recoveries are triggered by the risk evaluation, each replica will have to request the recovery.
In a \gls{smr} system the replicas must coordinate to execute the same requests.
One way is to assume that replicas have synchronous clocks, and then, the all the requests are synchronized.
However, we want to make the few timely assumptions, mainly because using the internet to communicate with \gls{osint} sources we are subject to different levels of delays.
Therefore, we propose a solution that uses a logical timeout protocol to (logically) synchronize replicas.
This protocol was firstly proposed by Kirsch~\etal{}~\cite{Kirsch:2014} as a solution to replicas coordinate their polling requests in the context os \gls{scada} systems.
The authors named this protocol as Logical Time Out protocol, and it can be briefly described as:
All \emph{Controller Replicas} run an algorithm where that periodically sends a \texttt{SYNC} message.
This message is used to inform other replicas of the current timeouts that a replica has.
Each replica will set their own timeouts by setting up a timeout \texttt{T$_x$} with the local time \emph{c$_i$} and the local expiration time \emph{$d$} to replica \emph{i}.
...
\paragraph{Distributed Random Protocol}
...
%We can use this algorithm to decide in a secure and distributed way which random configuration will replace other.
...
\note{Deixar claro o que e o throughput como ficou no sieveq}
...
\begin{figure}[h]
\subfloat[KVS state size $\approx 30$MBs.]{\includegraphics[width=\textwidth]{images/gnuplot/vagrant/reconfiguration/reconfiguration_state10mbs.pdf}\label{fig:s10mbs}}

\subfloat[KVS state size $\approx 60$MBs.]{\includegraphics[width=\textwidth]{images/gnuplot/vagrant/reconfiguration/reconfiguration_state50mbs.pdf}\label{fig:s50mbs}}
